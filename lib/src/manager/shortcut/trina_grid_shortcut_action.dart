...
class TrinaGridActionDefaultEnterKey extends TrinaGridShortcutAction {
  ...
  void execute({
    ...
    if (stateManager.configuration.enterKeyAction.isToggleEditing) {
      ...
      if (stateManager.currentColumn != null &&
          stateManager.currentRow != null &&
          stateManager.currentCell != null) {
        isReadOnly = stateManager.currentColumn!.checkReadOnly(
      ...
      if (stateManager.isEditing == true ||
          stateManager.currentColumn?.enableEditingMode?.call(
                stateManager.currentCell,
              ) ==
              false ||
          isReadOnly == true) {
        ...
        if (position != null &&
            ...
            position.columnIdx != null) {
          var nextCell = stateManager
              .refRows[position.rowIdx!]
              .cells[stateManager.refColumns[position.columnIdx!].field];
          if (nextCell != null) {
            bool isReadOnly = nextCell.column.checkReadOnly(
              stateManager.refRows[position.rowIdx!],
              nextCell,
            );
        ...
        if (saveIsEditing) {
          ...
          WidgetsBinding.instance.addPostFrameCallback((_) {
            if (stateManager.textEditingController != null) {
              stateManager.textEditingController!.selection = TextSelection(
                baseOffset: 0,
                extentOffset:
                    stateManager.textEditingController!.value.text.length,
              );
            }
          });
        }
  ...
  void _moveCell(
    ...
    if (enterKeyAction.isEditingAndMoveDown) {
      ...
      if (keyEvent.isShiftPressed) {
        ...
        final isAtLastRow = position.rowIdx! == stateManager.refRows.length - 1;

        if (isAtLastColumn && !isAtLastRow) {
        ...
        } else if (isAtLastColumn && isAtLastRow) {
          // At the last cell: insert a new row via moveCurrentCell if configured,
          // then jump directly to the first column of the new row.
          final rowCountBefore = stateManager.refRows.length;
          stateManager.moveCurrentCell(
            TrinaMoveDirection.down,
            force: true,
            notify: false,
          );
          // If a new row was inserted, move to its first column directly.
          if (stateManager.refRows.length > rowCountBefore) {
            final newRowIdx = position.rowIdx! + 1;
            final firstColumnField = stateManager
                .refColumns[columnIndexes.first].field;
            stateManager.setCurrentCell(
              stateManager.refRows[newRowIdx].cells[firstColumnField],
              newRowIdx,
              notify: false,
              direction: TrinaMoveDirection.down,
            );
            stateManager.moveScrollByRow(TrinaMoveDirection.down, position.rowIdx!);
...
class TrinaGridActionInsert extends TrinaGridShortcutAction {
  ...
  void execute({
    ...
    stateManager.insertRows(rowIdx, [stateManager.getNewRow()]);

    var newRow = stateManager.refRows[rowIdx];
    // Anem a la fila que hem creat
    var firstVisibleCol = stateManager.columns.firstWhereOrNull(
      (element) => !element.hide,
    );
    if (firstVisibleCol != null) {
      var cell = newRow.cells[firstVisibleCol.field];
      stateManager.setCurrentCell(cell, rowIdx, notify: true);
    }
  }
}
...
class TrinaGridActionSearch extends TrinaGridShortcutAction {
  ...
  void execute({
    ...
    if (stateManager.isEditing == true || stateManager.showLoading
    /*|| stateManager.mode == PlutoGridMode.readOnly
        || stateManager.currentCellPosition == null
        || stateManager.currentCellPosition?.rowIdx == null*/
    ) {
...
class TrinaGridActionSearchNext extends TrinaGridShortcutAction {
  ...
  const TrinaGridActionSearchNext({this.notFound, this.lastFound});
